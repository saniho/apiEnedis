            ...
            if ( self._myDataEnedis.getStatusLastCall()):  # update avec statut ok
                try:
                    ...
                    if (self._myDataEnedis.isProduction()):
                        state = status_counts['yesterday_production'] * 0.001
                    else:
                        state = status_counts['yesterday'] * 0.001
...
def _manageSensorState( _myDataEnedis,_LOGGER = None, version = None ):
    _LOGGER.info("call manageSensorState " )
    status_counts = defaultdict(int)
    status_counts["version"] = version

    if (_myDataEnedis.getContract() == None):
        try:
            _myDataEnedis.updateContract()
            _myDataEnedis.updateHCHP()
        except Exception as inst:
            _LOGGER.warning("myEnedis err %s" % (inst))
